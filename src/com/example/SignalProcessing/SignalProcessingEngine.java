...
public class SignalProcessingEngine {
	...
	public void init(Service context) {
		...
		Globals.sensorManager = (SensorManager) context
				.getSystemService(context.getApplicationContext().SENSOR_SERVICE);
		Globals.accSensor = Globals.sensorManager
				.getDefaultSensor(Sensor.TYPE_LINEAR_ACCELERATION);

		Globals.magneticSensor = Globals.sensorManager
				.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR);

		Globals.gravitySensor = Globals.sensorManager
				.getDefaultSensor(Sensor.TYPE_GRAVITY);

		listener = new AccelerometerListener();

		if (Globals.accSensor != null) {
			Globals.sensorManager.registerListener(listener, Globals.accSensor,
					SensorManager.SENSOR_DELAY_FASTEST);

			Globals.sensorManager.registerListener(new GravityListener(),
					Globals.gravitySensor, SensorManager.SENSOR_DELAY_FASTEST);

			Globals.sensorManager.registerListener(new MagnetometerListener(),
					Globals.magneticSensor, SensorManager.SENSOR_DELAY_FASTEST);
		}
	}
	...
	public static Vector3 integration(Queue<Vector3> buffer) {
		...
		for (int i = 1; i < buffer.size(); ++i) {
			...
			triangle = triangle
					.multiply((float) ((double) (current.timestamp - previous.timestamp) / 1000000000.0));
	...
	public boolean isAccDelimiter(Vector<Vector3> buffer) {
		...
		if ((float) (buffer.size() - val.size() + 1) / (float) buffer.size() > Globals.DELIMITER_ENTROPY) {
			return true;
		}
	...
	public static void mergeSimilarPoints(Vector<Vector3> gesture) {
		for (int i = 0; i < gesture.size() - 1; ++i) {
			if (gesture.elementAt(i).getDist(gesture.elementAt(i + 1)) < Math
					.pow(10, -2)) {
				gesture.remove(i);
				i--;
			}
	...
	public void checkForDelimiter(Vector3 currentRawValue) {
		...
		if (delimitersBuffer.size() > DELIMITER_BUFFER_SIZE) {
			...
			if (delimiter && !delimiterStatus) {
				...
				/*
				 * if((delimiterOptions | ACC_DELIMITER) > 0){ for(int i = 0; i
				 * < delimitersBuffer.size(); ++i){ if(gestureBuffer.size() < 1)
				 * break; gestureBuffer.remove(gestureBuffer.size()-1); } }
				 */
	...
	public void update(SensorEvent rawVal) {
		...
		currentRawValue = SignalProcessingEngine.thresholding(currentRawValue,
				lastMeasuredVal);
		...
		if (medianBuffer.size() > MEDIAN_BUFF_SIZE) {
			...
			if (medianBuffer.size() > 0) {
				sRaw = SignalProcessingEngine.thresholding(currentAccVal,
						getLast(speedSupportBuffer));
			}
	...
	public static int tresholdPass(Vector3 a, Vector3 b) {
		int axis = 0;
		if (Math.abs(a.x - b.x) > Globals.TRESHOLD) {
			axis |= OX;
		}
		if (Math.abs(a.y - b.y) > Globals.TRESHOLD) {
			axis |= OY;
		}
		if (Math.abs(a.z - b.z) > Globals.TRESHOLD) {
			axis |= OZ;
		}
		return axis;
	}
